...
class CarritoController {
    ...
    //nueva consulta
    public async actualizar_estado_carrito(req: Request, res: Response): Promise<void> {
        const { id } = req.params;
        const { nuevoEstado } = req.body;

        // Verificar si el nuevo estado es válido
        if (!['pendiente', 'en_proceso', 'entregado'].includes(nuevoEstado)) {
            res.status(400).json({ 'mensaje': 'El estado proporcionado no es válido' });
            return;
        }

        const resp = await pool.query('UPDATE carrito SET estado_carrito = ? WHERE id_carrito = ?', [nuevoEstado, id]);
        res.json(resp);
    }
}
